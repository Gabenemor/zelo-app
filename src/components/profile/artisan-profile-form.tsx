...
import { Phone, Mail, Home, Save, Coins, Tag, Image as ImageIcon, Briefcase, User, ArrowLeft, Info, UploadCloud, Camera, Activity, Edit, Loader2 } from "lucide-react";
...
const serviceExperienceSchema = z.object({
  ...
  years: z.coerce.number().int().min(0, "Years must be 0 or positive.").optional(),
...
export function ArtisanProfileForm({
  ...
  const handlePortfolioChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0 && userId) {
      ...
      setPortfolioPreviews(prev => [...prev.filter(url => !url.startsWith('blob:')), ...optimisticPreviews].slice(0, 5));

      try {
        const currentUploadedCount = uploadedPortfolioImageUrls.filter(url => !url.startsWith('blob:')).length;
        const filesToUpload = currentFileArray.slice(0, 5 - currentUploadedCount);

        if (filesToUpload.length > 0) {
          const newUploadedUrls = await uploadPortfolioImages(userId, filesToUpload);
          const updatedUrls = [...uploadedPortfolioImageUrls.filter(url => !url.startsWith('blob:')), ...newUploadedUrls].slice(0,5);
          setUploadedPortfolioImageUrls(updatedUrls);
          setPortfolioPreviews(updatedUrls);
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            <FormField control={form.control} name="username" render={({ field }) => (
                <FormItem>
                  ...
                  <div className="relative">
                    ...
                    <FormControl><Input placeholder="e.g. ZeloMasterArtisan" {...field} className="pl-10" disabled={isSubmitting}/></FormControl>
                  </div>
             ...
             <FormField control={form.control} name="headline" render={({ field }) => (
                <FormItem>
                  ...
                  <div className="relative">
                    ...
                    <FormControl><Input placeholder="e.g., Expert Plumber - 10+ Years Experience" {...field} className="pl-10" disabled={isSubmitting}/></FormControl>
                  </div>
          ...
          <FormField control={form.control} name="contactPhone" render={({ field }) => (
              <FormItem>
                ...
                <div className="relative">
                  ...
                  <FormControl><Input placeholder="e.g. +2348012345678" {...field} className="pl-10" disabled={isSubmitting}/></FormControl>
                </div>
                <FormMessage />
              </FormItem>
          )}/>
        ...
        {!isOnboarding && initialData?.servicesOffered && (
          <FormItem>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {initialData.servicesOffered.map(service => (<Badge key={service} variant="secondary" className="py-1 px-3 text-sm">{service}</Badge>))}
            </div>
            <FormDescription>To change your primary services (max 2), please <Link href="/dashboard/profile/artisan/services/edit" className="text-primary underline hover:text-primary/80">visit the service management section</Link>.</FormDescription>
          </FormItem>
        )}
        {fields.length > 0 && (
          <Card>
            ...
            {fields.map((item, index) => (
              <div key={item.id} className="space-y-4 rounded-md border bg-secondary/30 p-4">
                <FormField control={form.control} name={`serviceExperiences.${index}.serviceName`} render={({ field: serviceNameField }) => (<FormItem><FormLabel className="text-md font-semibold text-foreground">Service: {serviceNameField.value}</FormLabel></FormItem>)}/>
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                    <FormField control={form.control} name={`serviceExperiences.${index}.years`} render={({ field: yearsField }) => (
                        <FormItem className="sm:col-span-1">
                        <FormLabel>Years of Experience</FormLabel>
                        <div className="relative"><Briefcase className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" /><FormControl><Input type="number" placeholder="Years" min="0" {...yearsField} className="pl-10 pr-2 text-sm h-9" disabled={isSubmitting}/></FormControl></div>
                        <FormMessage className="text-xs"/>
                        </FormItem>
                    )}/>
                    <FormField control={form.control} name={`serviceExperiences.${index}.chargeAmount`} render={({ field: chargeAmountField }) => (
                        <FormItem className="sm:col-span-1">
                        <FormLabel>Charge Amount (₦)</FormLabel>
                        <div className="relative"><Coins className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" /><FormControl><Input type="number" placeholder="e.g. 5000" min="0" {...chargeAmountField} value={chargeAmountField.value ?? ''} className="pl-10 pr-2 text-sm h-9" disabled={isSubmitting}/></FormControl></div>
                        <FormMessage className="text-xs"/>
                        </FormItem>
                    )}/>
                    <FormField control={form.control} name={`serviceExperiences.${index}.chargeDescription`} render={({ field: chargeDescField }) => (
                        <FormItem className="sm:col-span-1">
                        <FormLabel>Charge Basis</FormLabel>
                        <div className="relative"><Tag className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" /><FormControl><Input placeholder="e.g. per hour" {...chargeDescField} value={chargeDescField.value ?? ''} className="pl-10 pr-2 text-sm h-9" disabled={isSubmitting}/></FormControl></div>
                        <FormMessage className="text-xs"/>
                        </FormItem>
                    )}/>
                </div>
        ...
        <FormField control={form.control} name="location" render={({ field }) => (
            <FormItem>
              ...
              <Controller name="location" control={form.control} render={({ field: locField }) => (
        ...
        <FormField control={form.control} name="isLocationPublic" render={({ field }) => (
            <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
              <div className="space-y-0.5"><FormLabel className="text-base">Share Precise Location</FormLabel><FormDescription>Allow clients to see your more specific location.</FormDescription></div>
              <FormControl><Switch checked={field.value} onCheckedChange={field.onChange} aria-label="Toggle precise location sharing" disabled={isSubmitting}/></FormControl>
            </FormItem>
        )}/>
        <FormField control={form.control} name="availabilityStatus" render={({ field }) => (
            <FormItem>
              ...
              <Select onValueChange={field.onChange} defaultValue={field.value} disabled={isSubmitting}>
                <FormControl>
                  <SelectTrigger className="relative pl-10"><Activity className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><SelectValue placeholder="Select your availability" /></SelectTrigger>
                </FormControl>
                <SelectContent>
        ...
        <FormField control={form.control} name="bio" render={({ field }) => (
            <FormItem>
              ...
              <FormControl><Textarea placeholder="Tell clients about yourself, your skills, and what makes your service stand out..." className="resize-y min-h-[100px]" {...field} disabled={isSubmitting}/></FormControl>
              ...
              <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
                {portfolioPreviews.slice(0,5).map((url, index) => (
                  <div key={index} className="relative aspect-[3/2] w-full overflow-hidden rounded-md border shadow-sm">
                    <Image src={url} alt={`Portfolio image ${index + 1}`} width={300} height={200} className="object-cover w-full h-full" data-ai-hint={url.includes("placehold.co") || url.startsWith("blob:") ? "portfolio work" : undefined}/>
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="flex flex-wrap justify-end gap-3 pt-4">
          {backButtonHref && (<Button asChild variant="outline" disabled={isSubmitting || isUploadingProfilePhoto || isUploadingPortfolio}><Link href={backButtonHref}><ArrowLeft className="mr-2 h-4 w-4" /> {backButtonText}</Link></Button>)}
